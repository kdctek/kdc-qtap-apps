...
A Grade-Specific Fee's slug badge (e.g. `visit-to-museum`) is now generated and shown immediately when you type the fee's name, instead of sticking on the `_custom_<index>` fallback until the page was reloaded. Previously the human slug was only (re)generated by the per-card **Save** button and the full-form save, so a fee whose name was set via blur-autosave kept the placeholder slug in the badge until a hard refresh. The slug is now regenerated on name autosave too, and the badge updates live on both the autosave and the Save-button paths.
