...
On the student/parent fees page, each **Grade-Specific Fee** card now shows:
- **Rich-text description** — the fee's description (entered in the admin Fee Matrix) is displayed on the card with basic formatting (bold, links, lists). The admin description field is now a multi-line box that accepts safe HTML (`wp_kses_post`); it is shown to students exactly as entered.
- **Copy payment link** — a button that copies a shareable deeplink to that specific fee: `https://{site}/{fees-page}/?year={year}&fee={slug}`. Opening the link scrolls to and opens that fee for payment. Uses the fee's human slug (e.g. `visit-to-museum`).

### Fixed — Grade-Specific Fee slug updates without a hard refresh (was 3.24.10)

The fee's slug badge (e.g. `visit-to-museum`) is now generated and shown immediately when you type the fee's name, instead of sticking on the `_custom_<index>` fallback until the page was reloaded. The slug is regenerated on name autosave, and the badge updates live on both the autosave and the per-card **Save** paths.

## [3.24.10] - 2026-07-14

### Fixed — Grade-Specific Fee slug now updates without a hard refresh

Superseded by 3.24.11 (released together). A Grade-Specific Fee's slug badge is generated and shown immediately when you type the fee's name, instead of sticking on the `_custom_<index>` fallback until a reload.
